...
(function() {
  ...
  function initEnhancedFriends() {
    ...
    if (!friendsModal || document.getElementById('friends-tabs')) return;

    const content = friendsModal.querySelector('.friends-content');
    if (!content) return;
    ...
    items.forEach(item => {
      ...
      el.innerHTML = `
        <div class="friend-info">
          <span class="friend-name">${item.username || item.reportedBy || 'Unknown'}</span>
          <span class="friend-status ${item.online ? 'online' : 'offline'}">
            ${item.online ? '🟢 Online' : '⚫ Offline'}
          </span>
        </div>
        <div class="friend-actions">
          <button class="friend-btn" onclick="challengeUser('${item.username || item.reportedBy}')">Challenge</button>
          ${tabId === 'friends-tab' ? '<button class="friend-btn" onclick="removeFriend(\'' + (item.username || item.reportedBy) + '\')">Remove</button>' : ''}
          ...
          ${tabId === 'blocked-tab' ? '<button class="friend-btn" onclick="unblockUser(\'' + (item.username || item.reportedBy) + '\')">Unblock</button>' : ''}
        </div>
      `;
  ...
  function createCheatModal() {
    ...
    document.getElementById('close-cheats-btn').addEventListener('click', () => {
      modal.classList.add('hidden');
    });
  }
