...
window.Debugger = {
    ...
    setupEventListeners() {
        ...
        document.getElementById('debugger-close')?.addEventListener('click', () => this.toggle());

        // Clear logs
        document.getElementById('debugger-clear')?.addEventListener('click', () => this.clearLogs());

        // Export logs
        document.getElementById('debugger-export')?.addEventListener('click', () => this.exportLogs());

        // File selector
        document.getElementById('debugger-file-select')?.addEventListener('change', (e) => {
        ...
        document.querySelectorAll('.debugger-tab').forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                document.getElementById(`debugger-${tab.dataset.tab}`).classList.add('active');
            });
        });

        // Console command input
        document.getElementById('debugger-cmd-btn')?.addEventListener('click', () => this.executeCommand());
        document.getElementById('debugger-cmd-input')?.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') this.executeCommand();
        });

        // Action buttons
        document.getElementById('debug-reset-game')?.addEventListener('click', () => {
            if (window.resetGame) window.resetGame();
            this.log('ACTION', 'Game reset via debugger');
        });

        document.getElementById('debug-toggle-bot')?.addEventListener('click', () => {
            if (window.toggleBotMode) window.toggleBotMode();
            this.log('ACTION', 'Bot mode toggled via debugger');
        });

        document.getElementById('debug-force-win')?.addEventListener('click', () => {
            this.log('ACTION', 'Force win triggered via debugger');
            // Add your force win logic here
        });

        document.getElementById('debug-test-move')?.addEventListener('click', () => {
            this.testRandomMove();
        });
    },
