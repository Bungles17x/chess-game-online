...
function createTutorialUI() {
  // Check if tutorial section already exists in settings.html
  const tutorialSection = document.getElementById("tutorial-section");
  
  if (tutorialSection) {
    console.log("Tutorial section found in settings.html");
    // Tutorial section already exists in settings.html, just add event listeners
    const basicsBtn = document.getElementById("tutorial-basics-btn");
    const strategiesBtn = document.getElementById("tutorial-strategies-btn");
    
    console.log("Basics button:", basicsBtn);
    console.log("Strategies button:", strategiesBtn);
    
    if (basicsBtn) {
      basicsBtn.addEventListener("click", () => {
        console.log("Basics button clicked");
        startTutorial("basics");
      });
    }
    if (strategiesBtn) {
      strategiesBtn.addEventListener("click", () => {
        console.log("Strategies button clicked");
        startTutorial("strategies");
      });
    }
    return;
  }
  
  // Add tutorial section to theme modal (for index.html)
  const themeModal = document.getElementById("theme-modal");
  if (themeModal) {
    const modalContent = themeModal.querySelector(".modal-content");
    if (modalContent) {
      const newTutorialSection = document.createElement("div");
      newTutorialSection.className = "theme-section tutorial-section";
      newTutorialSection.innerHTML = `
        <h3>Tutorial Mode</h3>
      ...
      const closeBtn = modalContent.querySelector("#close-theme-btn");
      if (closeBtn) {
        modalContent.insertBefore(newTutorialSection, closeBtn);
      } else {
        modalContent.appendChild(newTutorialSection);
      }
...
function createTutorialOverlay() {
  ...
  console.log("Tutorial overlay created and appended to body");
  console.log("Overlay element:", overlay);
  console.log("Title element:", document.getElementById("tutorial-title"));
  console.log("Text element:", document.getElementById("tutorial-text"));
...
function startTutorial(tutorialId) {
  ...
  if (!overlay) {
    console.error("Tutorial overlay not found after creation");
    return;
  }
...
function updateTutorialUI() {
  const tutorial = TutorialMode.tutorials[TutorialMode.currentTutorial];
  if (!tutorial) {
    console.error("Tutorial not found:", TutorialMode.currentTutorial);
    return;
  }

  const step = tutorial.steps[TutorialMode.currentStep];
  if (!step) {
    console.error("Step not found:", TutorialMode.currentStep);
    return;
  }

  console.log("Updating tutorial UI for step:", step.title);
  ...
  console.log("Elements found:", { titleEl, textEl, stepNumEl, totalStepsEl });

  // Update content
  if (titleEl) titleEl.textContent = step.title;
  if (textEl) textEl.textContent = step.content;
  if (stepNumEl) stepNumEl.textContent = TutorialMode.currentStep + 1;
...
function updateTutorialProgress() {
  const totalTutorials = Object.keys(TutorialMode.tutorials).length;
  const completed = TutorialMode.completedSteps.size;
  const percentage = (completed / totalTutorials) * 100;
